...
def main_post ():
    if request.method == 'POST': # index html de kutucuya girilen deger number olarak adlandirilmaktadir
        ...
        if  not alpha.isdecimal(): 
            return render_template('index.html', developer_name = 'Serkan_Sahan', not_valid= True)
        number= int(alpha)
        if not 0 < number < 4000: 
            return render_template('index.html', developer_name = 'Serkan_Sahan', not_valid= True)
        return render_template('result.html', number_decimal = number, number_roman = convert(number), developer_name = 'Serkan_Sahan')
    else : # girilen metod post degilse
        return render_template('index.html', developer_name = 'Serkan_Sahan', not_valid =False )
